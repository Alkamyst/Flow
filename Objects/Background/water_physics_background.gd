...
func create_particle():
	...
	trans.origin += Vector2(randf_range(0,1400),randf_range(-20,10))
	#physics body
	...
	ps.body_set_collision_layer(water_col,10)
	ps.body_set_collision_mask(water_col,10)
	...
	rect.size = particle_texture.get_size()/5
	...
	vs.canvas_item_set_self_modulate(water_particle,Color("000000ff"))
...
func _physics_process(_delta):
	...
	if spawn_timer < 0 and Globals.total_water_particles_background < max_water_particles:
		create_particle()
		current_particle_count += 1
		Globals.total_water_particles_background += 1
